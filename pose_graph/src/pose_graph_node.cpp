...
queue<nav_msgs::Odometry::ConstPtr> pose_buf;  // 当前帧的pose
...
int SKIP_CNT;  // 之后运行process()内循环的间隔
int skip_cnt = 0;
bool load_flag = 0;
bool start_flag = 0;
double SKIP_DIS = 0;  // 判断是否构建关键帧的距离标准

int VISUALIZATION_SHIFT_X;  // 可视化界面中图像x轴y轴的偏移量，一般设置为0
int VISUALIZATION_SHIFT_Y;  // 可视化界面中图像x轴y轴的偏移量，一般设置为0
int ROW;
int COL;
int DEBUG_IMAGE;
int VISUALIZE_IMU_FORWARD;
int LOOP_CLOSURE;  // loop_closure=1即进行回环检测
...
void imu_forward_callback(const nav_msgs::Odometry::ConstPtr &forward_msg)
{
    if (VISUALIZE_IMU_FORWARD)
    {
        ...
        vio_t = posegraph.w_r_vio * vio_t + posegraph.w_t_vio;  // 位姿矫正，把当前位姿从错误的世界坐标系纠正到正确的坐标系上
        vio_q = posegraph.w_r_vio *  vio_q;

        vio_t = posegraph.r_drift * vio_t + posegraph.t_drift;  // 位姿矫正，对短期位姿漂移的矫正
        vio_q = posegraph.r_drift * vio_q;

        Vector3d vio_t_cam;  // 把这个位姿从body->world变成cam->world
...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    if (!VISUALIZE_IMU_FORWARD)  // 用于可视化
    {
        cameraposevisual.reset();
        cameraposevisual.add_pose(vio_t_cam, vio_q_cam);
    ...
    odometry_buf.push(vio_t_cam);  // 把相机位姿的平移分类放到buffer里
    if (odometry_buf.size() > 10)
    {
        odometry_buf.pop();
    }

    visualization_msgs::Marker key_odometrys;  // key_odometrys的作用在vins中也是用于可视化的
    ...
    for (unsigned int i = 0; i < odometry_buf.size(); i++)  // 这个buffer是为了给key_odometrys提供相机运动过程中一系列空间坐标数据的
    {
...
void process()
{
    ...
    while (true)
    {
        ...
        // 构建pose_graph中用到的关键帧。这里用到的策略是：
        // 先剔除最开始的SKIP_FIRST_CNT帧，然后每隔SKIP_CNT，将距上一关键帧距离（平移向量的模）超过SKIP_DIS的图像创建为关键帧
        if (pose_msg != NULL)
        {
...
int main(int argc, char **argv)
{
    ...
    if (LOOP_CLOSURE)
    {
        ...
        if (LOAD_PREVIOUS_POSE_GRAPH)  // 按需求加载先前位姿图
        {
